@if (count($enlaces) > 0)
    <table class="" width="100%" cellspacing="1" style="color: black!important" style="border:1px dotted gray">
        <thead style="background-color: #919090;color:white;">
            <tr style="text-align:center;font-size:1.2em !important">
                <th style="width:5%">Id</th>
                <th style="width:5%">Título</th>
                <th style="width:12%">Nombre</th>
                <th style="width:12%">Apellido Paterno</th>
                <th style="width:12%">Apellido Materno</th>
                <th style="width:12%">Cargo</th>
                <th style="width:10%">Tipo de Enlace</th>                
                <th style="width:10%">Dependencia</th>                
                <th style="width:12%">Usuario</th>                
                <th style="width:10%">Contraseña</th>                
            </tr>
        </thead>
        <tbody>
            @php($i = true)
            @foreach ($enlaces as $enlace)                        
                <tr style="text-align:left" class="{{!$i?'ban':''}}">
                    <td style="width:5%">{{ $enlace->idEnlaceDependencia }}</td>
                    <td style="width:5%">{{ $enlace->titulo }}</td>
                    <td style="width:12%">{{ $enlace->nombre }}</td>
                    <td style="width:12%">{{ $enlace->apellidoP }}</td>
                    <td style="width:12%">{{ $enlace->apellidoM }}</td>
                    <td style="width:12%">{{ $enlace->cargo }}</td>
                    <td style="width:10%">{{ $enlace->tipoEnlace }}</td>                    
                    <td style="width:10%">{{ $enlace->dependenciaSiglas }}</td>                    
                    <td style="width:12%">{{ $enlace->cuenta }}</td>                    
                    <td style="width:10%;text-align:center">{{ base64_decode($enlace->enc) }}</td>                    
                </tr>
            @php($i=!$i)   
            @endforeach
        </tbody>
    </table>
@else
    <div class="text-center">
        <h3>
            No existen Enlaces Registrados!
        </h3>
    </div>
@endif
<style>
    table tr th {
        background-color: #681b2e;
        font-weight: bold;     
        color: white;
        height:20px;   
    }

    table{
        font-size: .8em
    }

    table tr td{
        vertical-align: baseline;
    }

    .ban{
        background-color: rgb(208, 208, 208);
    }
</style>
